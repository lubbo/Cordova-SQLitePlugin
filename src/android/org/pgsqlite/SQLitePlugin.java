...
import com.effetreseizero.tourmaps.MainActivity;
    ...
    private boolean executeAndPossiblyThrow(Action action, JSONArray args, CallbackContext cbc)
        ...
        switch (action) {
        	case tileKey:
                 ...
                 zoom = o.getString("zoom");
                 row = o.getString("row");
                 col = o.getString("col");
                 this.tileKey(Long.valueOf(zoom), Long.valueOf(col), Long.valueOf(row), cbc);
                 break;
            case open:
                o = args.getJSONObject(0);
...
//		final long index = (y << 36) | (z << 28) | (x << 0);
    ...
    private SQLiteDatabase openDatabase(String dbname, boolean createFromAssets, CallbackContext cbc) throws Exception {
        try {
            ...
            File dbfile = new File(((MainActivity)this.cordova.getActivity()).dbPath+'/'+dbname);
